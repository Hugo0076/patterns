...
void setup() {
  ...
  frameRate(30);
  // read in data 
  // this opens file, loads in the data, and then closes file
  ...
  for (int i = 0; i < 5; i++) {
    ...
    color cp = color(Integer.parseInt(rgb[0]), Integer.parseInt(rgb[1]), Integer.parseInt(rgb[2]));
...
class Shape {
  ...
  Shape (color c_, int x1_, int y1_, int x2_, int y2_, int thickness_) {
    // pass all variables 
    this.col = c_; 
  ...
  void disp() {
    ...
    int y2_ = y2;
    //thickness = 1; < testing
    float scale = (1000/float(tileCount))/float(gridSize);
